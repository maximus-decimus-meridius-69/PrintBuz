...
import { ORDER_AMOUNT, type CourseOption, type PosterFormValues, type YearOption } from "@/lib/types";
...
export function PosterUploadForm() {
  const [values, setValues] = useState<PosterFormValues>(initialValues);
    ...
    try {
      ...
      const checkout = new window.Razorpay({
        ...
        handler: async (response: Record<string, string>) => {
          ...
          payload.append("rollNumber", values.rollNumber);
          payload.append("department", values.department);
          payload.append("year", values.year);
          payload.append("course", values.course);
          payload.append("email", values.email);
          payload.append("section", values.section);
    ...
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col gap-3 rounded-3xl bg-stone-950 px-5 py-4 text-sm text-stone-100 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-xl font-semibold">Rs. {ORDER_AMOUNT} per poster</p>
        </div>
        <button
          className={clsx(
